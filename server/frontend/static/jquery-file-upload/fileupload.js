$(function () {
    ...
    window.parent.uploadDocCount = $('#files_list').length;
   
        //settings:
        $('#fileupload').fileupload('option', {
            url: window.location.href,
            maxFileSize: 52428800,
            acceptFileTypes: /(\.|\/)(pdf|xml|rixml|html|tiff|jpg|jpeg|png|bmp|gif|xbm|ppt|pptx|doc|docx|txt)$/i,
            resizeMaxWidth: 1920,
        ...
        $('#fileupload').bind('fileuploadsubmit', function (e, data) {
            var proj_guid= window.parent.upload_proj_guid;
            data.formData = {p_guid: proj_guid, _xsrf: getCookie('_xsrf')};
        });
            ...
            // Callback for successful uploads:
                ...
                if (data.context) {
                    data.context.each(function (index) {
                        ...
                        try
                        {
                            var doc = data.result[0].documents[0];
                            /*window.parent.documentsView.collection.add(
                                    new Document({name:doc.title,
                                                guid:doc.guid,
                                                owner:doc.owner,
                                                project:doc.project,
                                                mime_type:doc.mime_type,
                                                id: doc.id,
                                                ann_count: doc.ann_count,
                                                c_count: doc.c_count,
                                                status: doc.status}));*/
                            window.parent.projectsView.collection.setDocs(doc, window.parent.upload_proj_guid);
                        }
                        catch (err) {
                            //console.log(err);
                        }
