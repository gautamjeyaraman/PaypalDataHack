$(function () {
    ...
    window.Img = '';
        ...
        /*$('#fileupload').bind('fileuploadpreviewdone', function (e, data) {
            window.Img = $(e.originalEvent.target).children();
        });*/
        ...
        //Event handlers for cropping the profile image

        $("#save_cropped_image").click(function(){
            var profile_pic_loader = document.getElementById('profile_pic_loader');
            profile_pic_loader.style.display = "block";
            $('#fade-bg').fadeOut();
            var url = "/api/latest/user/avatar/edit";
            var selection = window.cropPicHandler.getSelection();
            selection['img_width'] = $("#crop_profile_pic").width();
            selection['img_height'] = $("#crop_profile_pic").height();
            var params = {'_xsrf': getCookie('_xsrf'), 'crop': JSON.stringify(selection)};
            $.post(url, params).then(function(res){
                window.cropPicHandler.setOptions({'hide': true, 'disable': true});
                var pro_pic = $('#user_profile_pic');
                pro_pic.removeClass('user_profile_pic');
                pro_pic.css('max-width', 'none');
                pro_pic.attr('src', '/api/latest/user/avatar/edit?_xsrf=' + encodeURIComponent(getCookie("_xsrf")) + '&date=' + new Date().getTime());
                var scaleX = 150 / (selection.width || 1);
                var scaleY = 150 / (selection.height || 1);
                var imgHeight = selection['img_height'];
                var imgWidth = selection['img_width'];
                pro_pic.css({
                ...
                profile_pic_loader.style.display = "none";
            });    
        });

        $("#cancel_cropped_image").click(function(){
            window.cropPicHandler.setOptions({'hide': true, 'disable': true});
            $('#fade-bg').fadeOut();
            var url = "/api/latest/user/avatar/edit?_xsrf=" + getCookie("_xsrf");
...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function (e, data) {
                $("#user_profile_pic").css('display', 'none');
		...
		//#that._adjustMaxNumberOfFiles(-data.files.length);
		    ...
		    switch(et)
		    {
			    case 'acceptFileTypes':
				    errmsg = 'Invalid Image Type. Only jpg|jpeg|png|bmp|gif image types are allowed';
				    break;
			    case 'maxFileSize':
				    errmsg = "Image is too big to upload. Try a smaller image (2 MB)"; 
				    break;
			    default:
				    errmsg = 'Invalid image file selected ' + et;
				    break;
		    }
		    ...
		    //spanTag.style.color = "red";
                    ...
                    window.setTimeout(function(){
                        var profile_pic_loader = document.getElementById('profile_pic_loader');
                        profile_pic_loader.style.display = "none";
                        //choose_btn.removeChild(spanTag);
			errelt.css('display', 'none'); 
			errelt.html(''); 
            ...
            // Callback for successful uploads:
                ...
                var profile_pic = document.getElementById('crop_profile_pic');
                profile_pic.src = '/api/latest/user/avatar/edit?_xsrf=' + encodeURIComponent(getCookie("_xsrf")) + '&date=' + new Date().getTime();
                document.getElementById('cropped_profile_pic').src = profile_pic.src;
                var profile_pic_loader = document.getElementById('profile_pic_loader');
                profile_pic_loader.style.display = "none";
                profile_pic.onload = function(){
                    //For the crop option
                    var minSize = ($(profile_pic).height() > $(profile_pic).width())? $(profile_pic).width()/2: $(profile_pic).height()/2;
                    if(typeof window.cropPicHandler == "undefined"){
                        window.cropPicHandler = $(profile_pic).imgAreaSelect({
                                                                handles: false,
                                                                aspectRatio: "1:1",
                                                                onSelectChange: that.crop_preview,
                                                                instance: true,
                                                                persistent: true,
                                                                x1: (minSize-100 < 0)?0:minSize-100,
                                                                y1: (minSize-100 < 0)?0:minSize-100,
                                                                x2: minSize,
                                                                y2: minSize
                                                            });
                    }
                    else{
                        window.cropPicHandler.setOptions({hide: false,
                                                          disable: false,
                                                          handles: false,
                                                          aspectRatio: "1:1",
                                                          onSelectChange: that.crop_preview,
                    ...
                    that.crop_preview(profile_pic, window.cropPicHandler.getSelection());
                };
            },
        ...
        _startHandler: function (e) {
            ...
            setTimeout(function() {
                $('.avatar_upload_msg').fadeOut('slow');
            }, 3000);
        },
 	...
 	_initTemplates: function () {
        ...
        crop_preview: function(img, selection) {
            var scaleX = 120 / (selection.width || 1);
            var scaleY = 120 / (selection.height || 1);
            var imgHeight = $(img).height();
            var imgWidth = $(img).width();
            $('#cropped_profile_pic').css({
                width: Math.round(scaleX * imgWidth) + 'px',
                height: Math.round(scaleY * imgHeight) + 'px',
                marginLeft: '-' + Math.round(scaleX * selection.x1) + 'px',
                marginTop: '-' + Math.round(scaleY * selection.y1) + 'px'
            });
            $(".imgareaselect-selection").siblings().css('height', selection.height);
            $(".imgareaselect-selection").siblings().css('width', selection.width);
        }
    });
