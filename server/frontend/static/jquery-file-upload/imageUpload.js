    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function (e, data) {
		        ...
		        if($.inArray("#tasks", window.location.href.split('/')) != -1){
		            window.elem_id = '#rec_status_task';
                    $(window.elem_id).css({'left':'6%', 'top':'93%', 'width':'60%'});
                }
                else if($.inArray("#overview", window.location.href.split('/')) != -1){
                    window.elem_id = '#rec_status';
                    $(window.elem_id).css({'left':'6%', 'top':'93%', 'width':'70%'});
                }
                else if(that.element[0].id == 'docImgCmt'){
                    window.elem_id = '#imgCmtErrDocViewer';
                }
                $(window.elem_id).css({'color':'black'});
		...
		//#that._adjustMaxNumberOfFiles(-data.files.length);
		    ...
		    switch(et)
		    {
			    case 'acceptFileTypes':
				    ...
				    break;
			    case 'maxFileSize':
				    errmsg = "Image size is too big to upload. Try a smaller image(Less than 5 MB)."; 
				    break;
			    default:
            ...
            $(window.elem_id).html(errmsg);
            $(window.elem_id).css({'color':'red'});
            ...
            send: function (e, data) {
                ...
                $(window.elem_id).css({'color':'black'});
                $(window.elem_id).html('Your image comment is being uploaded...');
            },
            ...
            // Callback for successful uploads:
                ...
                window.elem_id = '';
                /*window.setTimeout(function(){
                    var profile_pic = document.getElementById('user_profile_pic');
                    if(profile_pic){
                        profile_pic.src = '/api/latest/user/avatar?_xsrf=' + encodeURIComponent(getCookie("_xsrf")) + '&guid=' + encodeURIComponent(window.user_guid) + '&date=' + new Date().getTime();
                    }
                    window.setTimeout(function(){
                        var profile_pic_loader = document.getElementById('profile_pic_loader');
                        profile_pic_loader.style.display = "none";
